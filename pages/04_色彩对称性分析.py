...
class ColorSymmetryAnalyzer:
    ...
    def analyze(self, image_input: np.ndarray):
        ...
        return {"score": round(score, 2), "med": round(med, 2), "heatmap": heatmap_color}
...
if uploaded_file:
    ...
    with col2:
        st.subheader("不对称热力图")
        st.image(cv2.cvtColor(result['heatmap'], cv2.COLOR_BGR2RGB), caption="红色=差异大，蓝色=差异小", use_container_width=True)
    with col3:
        st.subheader("分析报告")
        st.metric("色彩对称评分", f"{result['score']} / 100", delta="完美镜像" if result['score'] > 95 else None)
        st.metric("平均像素色差 (MED)", f"{result['med']}", delta="-越低越好", delta_color="inverse")
        st.info("如果评分低但热力图显示只有局部是红色，说明大部分区域是对称的，只有局部物体破坏了对称性。")